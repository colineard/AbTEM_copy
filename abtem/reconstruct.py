...
class RegularizedPtychographicOperator(AbstractPtychographicOperator):
    ...
    def preprocess(self):
        ...
        # Convert DiffractionPatterns Objects
        if isinstance(self._diffraction_patterns, DiffractionPatterns):
            (
    ...
    def reconstruct(
        ...
        **kwargs,
        ...
        Returns
        -------
        reconstructed_object_measurement: DiffractionPatterns or Sequence[DiffractionPatterns]
        ...
        if return_iterations:
            ...
            objects, probes, positions, sse = list(map(list, zip(*results)))
            objects = stack(objects, axis_metadata)
            probes = stack(probes, axis_metadata)

            return objects, probes, positions, sse
        else:
    ...
    def _prepare_measurement_outputs(
        ...
        Returns
        -------
        objects_measurement: DiffractionPatterns
        ...
        measurement_objects = Images(asnumpy(objects), sampling=self.sampling)
        measurement_probes = Images(asnumpy(probes), sampling=self.sampling)
...
class SimultaneousPtychographicOperator(AbstractPtychographicOperator):
    ...
    def preprocess(self):
        ...
        for dp in self._diffraction_patterns:
            ...
            if isinstance(dp, DiffractionPatterns):
                (
                    _dp,
                    angular_sampling,
                    step_sizes,
        ...
        if self._probes is None:
            ...
            if isinstance(self._probes[0], Probe):
                if self._probes[0].gpts != self._region_of_interest_shape:
                    raise ValueError()
                self._probes = tuple(
                    copy_to_device(_probe.build(lazy=False).array, self._device)
    ...
    def _prepare_measurement_outputs(
        ...
        measurement_objects = tuple(
            Images(asnumpy(_object), sampling=self.sampling) for _object in objects
        )
        measurement_probes = tuple(
            Images(asnumpy(_probe), sampling=self.sampling) for _probe in probes
        )
...
class MixedStatePtychographicOperator(AbstractPtychographicOperator):
    ...
    def _prepare_measurement_outputs(
        ...
        measurement_objects = Images(asnumpy(objects), sampling=self.sampling)
        measurement_probes = [
            Images(asnumpy(probe), sampling=self.sampling) for probe in probes
        ]
...
class MultislicePtychographicOperator(AbstractPtychographicOperator):
    ...
    def _prepare_measurement_outputs(
        ...
        ensemble_axes_metadata = [ThicknessAxis(values=slice_thicknesses)]

        measurement_objects = Images(
            asnumpy(objects),
            sampling=self.sampling,
            ensemble_axes_metadata=ensemble_axes_metadata,
        )
        measurement_probes = Images(
            asnumpy(probes),
            sampling=self.sampling,
            ensemble_axes_metadata=ensemble_axes_metadata,
        )
